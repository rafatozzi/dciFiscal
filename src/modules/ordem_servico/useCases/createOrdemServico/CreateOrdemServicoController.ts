...
export class CreateOrdemServicoController {
  async handle(request: Request, response: Response): Promise<Response> {
    const { id, descricao, id_cliente, id_user, id_empresa } = request.body as ICreateOrdemServicoDTO;

    const useCase = container.resolve(CreateOrdemServicoUseCase);

    const result = await useCase.execute(request.cod_cliente, { id, descricao, id_cliente, id_user, id_empresa });
